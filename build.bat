...
set BULLET_PATH=%GAME_PATH%/bullet-physics
set PATH=%PATH%;%OIS_PATH%/lib;%OGRE_PATH%/build/bin;%BOOST_PATH%/lib;%ZLIB_PATH%;%FREEIMAGE_PATH%;%BASS_PATH%;%CEGUI_PATH%/bin;%LUA_BRIDGE_PATH%;%BULLET_PATH%
make -j 4 DRIVE_LETTER="%DRIVE_LETTER%" OIS_PATH="%OIS_PATH%" PROJECT_DIR="%PROJECT_PATH%" OGRE_PATH="%OGRE_PATH%" BASS_PATH="%BASS_PATH%" LUA_BRIDGE_PATH="%LUA_BRIDGE_PATH%" BULLET_PATH="%BULLET_PATH%"
if %errorlevel% neq 0 (
